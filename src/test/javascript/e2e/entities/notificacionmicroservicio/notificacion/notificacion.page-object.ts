...
export class NotificacionUpdatePage {
  ...
  fechaSeedInput = element(by.id('field_fechaSeed'));
  ...
  async setFechaSeedInput(fechaSeed: string): Promise<void> {
    await this.fechaSeedInput.sendKeys(fechaSeed);
  }

  async getFechaSeedInput(): Promise<string> {
    return await this.fechaSeedInput.getAttribute('value');
  }
